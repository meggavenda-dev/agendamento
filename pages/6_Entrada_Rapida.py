...
st.caption("Digite em uma linha: 'Reunião amanhã 15h #trabalho !1 (30min) semanal qua,sex'")

text = st.text_input("Comando", "Enviar relatório sexta 14:30 #trabalho !2 (45min) semanal qua,sex")

def parse(t: str):
    o = {
        "title": t, "tag": None, "priority": 3, "estimated": 30, "due_local": None,
        "type": "task", "recurrence": "none", "recur_interval": 1, "recur_weekdays": None,
    }
    m = re.findall(r"#(\w+)", t)
    ...
    due = None
    if re.search(r"\bamanhã\b", t, re.I) or re.search(r"\bamanha\b", t, re.I):
        due = base + timedelta(days=1)
    elif re.search(r"\bhoje\b", t, re.I):
        due = base
    ...
    for k, v in dias.items():
        if re.search(rf"\b{k}\b", t, re.I):
            ...
            due = base + timedelta(days=delta); break

    m = re.findall(r"\b(\d{1,2})/(\d{1,2})\b", t)
    if m:
        d, mo = map(int, m[0]); y = base.year
        due = (due or base).replace(year=y, month=mo, day=d)
        t = re.sub(r"\b\d{1,2}/\d{1,2}\b", "", t)

    m = re.findall(r"\b(\d{1,2})(?::|h)?(\d{2})?h?\b", t)
    if m:
        hh = int(m[0][0]); mm = int(m[0][1] or 0)
