...
from db.visits import list_by_local_date, delete as visits_delete
from db.clinics import get_by_id as clinics_get_by_id
from core.scheduler import build_slots, filter_available_slots
from services.scheduling import create_visit, reschedule_visit
...
visits = list_by_local_date(selected_date, tz_name)
existing = []
for v in visits:
...
if st.button("Agendar visita", type="primary"):
    ...
    try:
        create_visit(int(clinic_id), start_dt, end_dt, existing)
...
if existing:
    ...
    if action == "Reagendar":
        ...
        if st.button("Confirmar reagendamento"):
            ...
            try:
                reschedule_visit(int(vid), new_start, new_end, others)
            except Exception as e:
                st.error(str(e))
                st.stop()
