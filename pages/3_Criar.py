...
def _now_local_rounded(hours_ahead: int = 2):
    """Retorna data/hora futura arredondada para minutos."""
...
def _compose_dt_from_date_time(date_value, time_value, fallback_hours=2):
    """Fallback para ambientes sem st.datetime_input."""
...
# Identificar se a versão suporta datetime_input
...
with st.form("create", clear_on_submit=False):
    ...
    # --------------------------------------
    # Entrada de data/hora (com fallback)
    # --------------------------------------
    ...
    recurrence_pt = st.selectbox(
        "Regra",
        list(recurrence_labels.keys()),
        index=0
    )

    recurrence = recurrence_labels[recurrence_pt]   # valor real salvo no banco
...
if submitted:
    ...
    item = {
        ...
        "recurrence": recurrence,             # valor em inglês no banco
