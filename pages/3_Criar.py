...
st.set_page_config(page_title="Criar", layout="wide")
...
with st.form("create"):
    ...
    due_local = st.datetime_input("Prazo/Horario", value=datetime.now(tz)+timedelta(hours=2))
    estimated = st.number_input("Estimado (min)", min_value=5, max_value=480, value=30, step=5)
    recurrence = st.selectbox("Recorrencia", ["none","daily","weekly","monthly","workdays","every_x_days"], index=0)
...
if submitted:
    if not title.strip(): st.error("Titulo obrigatorio."); st.stop()
    ...
    try:
        sb.table("items").insert(item).execute(); st.success("Criado com sucesso")
