...
def get_profile(sb, user_id):
    ...
    try:
        d = (
            sb.table("profiles")
              .select("timezone,theme,email_notifications,whatsapp_notifications,whatsapp_number")
              .eq("id", user_id)
              .single()
              .execute()
              .data
        ...
        if isinstance(d, dict):
            d.setdefault("timezone", "America/Sao_Paulo")
            d.setdefault("theme", "zen")
            return d
...
def week_range_for_tz(tz_name: str):
    ...
    start_l = (now_l - timedelta(days=now_l.weekday())).replace(
    ...
    try:
        res = (
            sb.table("items")
              ...
              .eq("user_id", user_id)
              .neq("status", "done")
              .or_(f"due_at.lt.{n_iso},and(due_at.gte.{n_iso},due_at.lte.{next_iso}),priority.lte.2")
